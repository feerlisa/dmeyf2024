...
HT_tuning_base <- function( pinputexps, bo_iteraciones, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    ...
    max_bin = 64L, # lo debo dejar fijo, no participa de la BO
    ...
    extra_trees = FALSE,
    # Parte variable
    bagging_fraction = seq(0.1, 1, by = 0.02),  # Crea la secuencia de 0.1 a 1
    pos_bagging_fraction = seq(0.1, 1, by = 0.02),  # Crea la secuencia de 0.1 a 1
    neg_bagging_fraction = seq(0.1, 1, by = 0.02),  # Crea la secuencia de 0.1 a 1
    skip_drop = seq(0.1, 1, by = 0.02),, # 0.0 <= skip_drop <= 1.0
    drop_rate = seq(0.1, 1, by = 0.02),, # 0.0 < neg_bagging_fraction <= 1.0
